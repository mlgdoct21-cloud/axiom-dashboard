/**
 * Telegram deep-link auth proxy.
 *
 * POST /api/auth/telegram-token  body: { token: string }
 * → Backend exchanges one-time token for JWT pair.
 */
...
export async function POST(request: NextRequest) {
  ...
  try {
    const r = await fetch(`${backendUrl}/auth/telegram-token`, {
      method: 'POST',
